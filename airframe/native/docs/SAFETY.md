...
**v0.1.4 exception to the legacy loss behavior below:** an explicitly selected, foreground-scoped Desktop Practice recovery session may freeze/release and reacquire for a fixed 30-second first-loss lease. Ordinary Start modes are unchanged. A return needs a new 500 ms near-pose open dwell, or after 1.25 seconds a one-second stronger four-finger-open dwell. The resume frame emits nothing and never restores an old drag. See [the v0.1.4 contract, limits and acceptance checks](RELEASE-0.1.4.md). Scope/focus/session checks are not an OS-input sandbox or universal lock detector. Camera faults, malformed or stale data, physical input, explicit stops and permission changes still hard-stop in every mode.
...
- Ordinary clicking/dragging mode still disarms on the very first missing observation and releases an owned button exactly once. The explicit v0.1.4 practice mode instead freezes and releases before bounded reacquisition. A stale sample or inference fault must never masquerade as an ordinary missing-hand observation, and queued good observations must not overwrite a pending fault.
- v0.1.3 adds a distinct thumb-only-uncertain observation, not a missing-hand exception: wrist/middle/index must each remain at least 0.70 confident with valid geometry. Only an active click session with no button held can enter amber frozen HOLD for at most 1.25 s. Every partial frame resets gesture readiness, and a fresh 500 ms open-hand dwell near the last point is required before resuming. A returning pinch cannot click. Full-hand loss and uncertainty during an owned drag still hard-stop. See [the detailed acceptance boundary](RELEASE-0.1.3.md).
...
| Missing hand in ordinary click-and-drag mode | Immediate disarm and owned-button release; no recovery | Synthetic tests; obstructed-camera trial later |
| Missing hand in explicit Desktop Practice recovery | Freeze/release once; fixed first-loss lease, new open dwell, zero-input resume, no inherited drag | New synthetic recovery tests; supervised hand trial still required |
