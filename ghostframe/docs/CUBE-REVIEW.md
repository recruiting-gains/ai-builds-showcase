...
The coordinator ran the revised fixed-production-build browser suite: **21/21 checks passed**, with no recorded page errors or mutation requests. The suite uses synthetic camera pixels and injected landmarks; the app still performs actual WebGL composition, MediaRecorder encoding, file download and decoding of the downloaded bytes. Local report: ignored `test-results/cube-hold-final/report.json`.

The new 263,155-byte MP4 covers six stages: small cube, enlarged cube, one-hand hold, carried movement, returning pair, and a bright background. In decoded frames, the central 98% blue-signal span stayed at 0.1758 of the video for sizing and 0.1758 while holding, then 0.1758 while moving; its untrimmed centroid moved from 0.4936 to 0.3740. Returning the pair enlarged the blue-signal span to 0.2799. These measurements are synthetic image-space evidence, not a measurement of real hand accuracy. The bright-frame test uses an orange stripe to check that the camera remains visible through the center; background blue pixels are not used as cube geometry evidence there.

The first GitHub Linux run passed 20/21 Cube gates, failing only the decoded spreading-versus-sized width assertion. The earlier replay test selected file frames using elapsed time since MediaRecorder.start, an unverified correspondence with encoded media time. Local Chrome and Chromium both passed that version, so the exact Linux offset was not established. The revised test embeds mirror-symmetric stage markers in synthetic camera pixels only after the pose settles, then discovers them in downloaded-file playback. It selects the second distinct decoded frame for each stage and requires chronological order, with a 20-second/360-callback limit. Geometry and transparency assertions were not weakened. The revised local Chromium run passes 21/21.

The marker-based Linux run also found all six stages but exposed a different measurement problem: nearly identical blue counts and centroids accompanied several-fold increases in raw minimum/maximum width. Sparse qualifying outliers can produce that pattern; their precise codec/pixel origin was not observed. The final saved-frame measurement uses the central 98% horizontal blue-pixel mass, reports raw span alongside it, and requires at least 98% of the original signal and 100 blue pixels to remain. No expected-position crop is applied. The independent critic accepted this estimator while requiring unchanged scale ratios, untrimmed centroid/movement checks, fixed second decoded frames and background/edge thresholds. A synthetic histogram probe checked resistance to distant outliers and translation, and the final local Chromium suite passes 21/21. This is a blue-signal span, not an exact outer-cage boundary.
