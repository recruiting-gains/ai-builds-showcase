...
const root = new URL('../', import.meta.url), base = 'http://127.0.0.1:8808';
const server = spawn(process.execPath, ['node_modules/vite/bin/vite.js', 'preview', '--host', '127.0.0.1', '--port', '8808', '--strictPort'], { cwd: root, stdio: ['ignore','pipe','pipe'] });
let child, log = '';
...
try {
  ...
  for (let attempt = 0; attempt < 40; attempt++) {
    if (server.exitCode !== null) throw new Error(`Panel preview exited: ${log}`);
  ...
  if (!ready) throw new Error(`Panel preview was not ready: ${log}`);
  child = spawn(process.execPath, ['--import', 'tsx', 'scripts/panel-browser-check.mjs', base], { cwd: root, stdio: 'inherit' });
