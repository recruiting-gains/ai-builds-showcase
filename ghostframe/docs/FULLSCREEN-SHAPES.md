...
With Follow my hands on, form the opening directly. Bring both palms together, pause for **“Hands together · reopen for the next color.”**, then reopen to advance one world, including in fullscreen. The first opening only starts the session; the new world's name confirms each completed change. Fingertip contact alone keeps the selected world. If one palm briefly hides the other near a recognized close, **“Reopen both hands now for the next color.”** signals a recovery window of at most 700 ms from the last measured pair. If tracking is lost for longer or both hands disappear, open both hands to re-arm before closing and reopening again. The world buttons and Prepare a still remain available on the main page. Pinch shortcuts stay disabled in this mode. With it off, two-hand position/depth controls move the saved outline and the quick/0.6-second pinch shortcuts remain available. Mouse controls provide a separate fallback. Center depth establishes a neutral palm-size reference; depth remains an estimate affected by palm orientation.
...
The preceding palm/response update passed **118 application tests, including 25 contour/perspective tests**. Its expanded fullscreen suite passed **19 cases**, including prefixed APIs, transient/stalled requests, focus restoration after native exit, optional API failures and transient versus sustained page hiding. That release's deployed assets matched its tested build. A separate embedded-browser simulated-preview check passed full-document expansion, Fill view, Just camera and exit. These are historical results; [current palm-contact acceptance and evidence](WORLD-CYCLE-WIDE-VIEW.md#verification) are tracked separately.
