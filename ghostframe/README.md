...
For **Cube**, start your camera and show two open hands. Their midpoint moves the cube and their separation changes its size. Briefly pinch one thumb and index finger, then release for the next appearance. Holding cannot prepare a still. **Move it myself** enables dragging, arrow keys and position/size sliders; **Appearance** also switches the look. Cube uses the existing local recorder. [Cube guide and verification](docs/CUBE.md).
