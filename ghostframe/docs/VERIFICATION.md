...
After the move, **214 application tests**, **14 harness tests**, frontend/Worker typechecks, the production build, **31 phone browser checks** and the deployment dry run pass. A separate browser check verifies the new title, header, footer and source link at desktop and phone widths; the showcase screenshot was captured from the renamed app with the camera off. Phone checks use generated camera input and synthetic hand landmarks, not a physical phone.
...
The application is deployed at [GhostFrame](https://jedi-mindtrick.recruiting-gains.workers.dev). The original release deployment is Cloudflare version `82a11722-2467-42a0-baa3-9d61fb217402`. Live HTML, health/config responses and same-origin model assets loaded successfully. At the original observation, source was on `codex/jedi-mindtrick` in [pull request 10](https://github.com/recruiting-gains/ai-builds-showcase/pull/10). That PR was subsequently merged into `main`; this paragraph records the original deployment.
...
## Close/reopen world cycle and wider HandFrame studio

HandFrame preserves the selected world on its first opening, then advances one world per confirmed hands-together/reopen sequence. Palm/wrist separation, short dwell, hysteresis and actual wrist-span contraction/expansion separate it from fingertip designs and depth-only motion. Loss or control changes cancel pending cycles. The camera now owns the studio row, with its existing buttons/status above and design controls below. Fullscreen starts fitted to preserve the full image.

**131 application tests and 77 local browser checks passed**, including 13 new state-machine cases and 12 focused integration/layout cases. At 1440×1050 the scene expanded from 954 to 1350.4 pixels wide; the source's four mirrored edge colors remained visible. Two complete cycles changed both selected controls and actual interior pixels exactly twice. Camera identity, normalized pointer alignment, mode/manual/follow/world/camera-only resets, camera restart, fullscreen Fit and 390/760/1440 layouts passed, with no page errors, uploads or unintended still preparation. Existing saved-shape, automatic-outline, fullscreen and behavior suites also passed. [Implementation and evidence](WORLD-CYCLE-WIDE-VIEW.md).

Run `1789102213266-9211449d-ddac-488b-89fb-142e12dee636` passed all six command nodes on attempt 1. Source fingerprint: `3bad6d1bfa0fdabd2a55e113d252ed68a77b217d817f6d2aa64af4c4c9c88456`. Cloudflare version `d88b2e83-b971-425e-8040-b3163dfc1cf9` is deployed. Live HTML and health returned HTTP 200; HTML, JavaScript, CSS and the vision worker exactly match the tested build. This update does not increase the lens field of view or claim physical-camera recognition/latency acceptance. No paid AI call or private camera/reference-media upload was used.

## Palm-contact recovery — September 11, 2026

The preceding world-cycle release rejected valid edge-on/overlapping palms and cancelled arming on every missing-hand detection. This correction accepts plausible palm depth geometry, matches detections by position, and adds bounded recovery: 300 ms for an already-armed opening before contact, and 700 ms near observed contact. Neither permits a lost hand alone to request a color change. The on-camera cue now confirms when to reopen, including fullscreen; the world name briefly confirms completion.

**146 application tests, 14 harness tests and 81 local browser checks passed.** The latter include 16 gesture/layout cases with prayer-palm contact, pre-contact dropout, contact occlusion, expiry and fresh recovery. Cue visibility, full camera edges, 390/760/1440-pixel layouts, saved shapes, automatic outlines, fullscreen, manual overrides, camera restarts and actual color pixels passed. No uncaught page errors were observed; gesture-only cases produced no still or upload. The project's dependency audit reports zero vulnerabilities. This does not certify dependency audits in unrelated repository applications.

The command harness passed six nodes on attempt 1: run `1789108279749-da71f874-9ac0-4e50-82eb-125c54bf4cde`, fingerprint `da4923064b6378d3380eebe900c8c010c0518b01abd9641c451de490860477f4`. Its physical-camera and real-provider gates remain separate from automated checks.

An independent reviewer reran all 28 focused gesture tests. A local replay of 484 detections from supplied, already-rendered footage completed three cycles versus one with the preceding controller. That demonstrates specific recoveries in that recording; it is not a recognition success rate or a live iPhone trial. Some attempts in the recording still lose both hands or lose tracking for too long. The reference media and extracted landmarks remain private and are not in this repository. See [gesture contract, limits and reproducible checks](WORLD-CYCLE-WIDE-VIEW.md).

## Opposing L orientations — September 11, 2026

HandFrame now connects an upright L with an upside-down L on either side. It considers the two endpoint pairings of the existing measured finger chains and selects a valid opening. Crossed chains still hide the effect, and changing the connection resets smoothing so old contour indices cannot distort the next pose. Turning a hand over preserves the selected world. [Behavior and focused checks](AUTOMATIC-HAND-SHAPES.md).

The former build failed the first opposing-L browser fixture after passing three existing checks. The corrected build passes **169 application tests, 14 harness tests, 12 automatic-outline browser checks and 10 recording browser checks**. The new browser cases cover either inverted hand, detector reordering, a sloped pair, upright/inverted transitions, selected-world preservation, actual filtered pixels and phone fullscreen/camera-only controls. Existing depth, invalid-input recovery, saved shapes, mocked still display and camera restart also pass. Recording uses actual Chrome encoding with a generated camera; native file sharing is mocked. These are synthetic-gesture and browser integration results, not physical iPhone recognition or latency measurements.

Run `1789160517269-f9b1a749-bc1e-4994-bd76-4e9156e8d0c7` passed all six command nodes on attempt 1, fingerprint `6634eacfb1eb44974ddbfb568112075ba15b54cc5b6b4c11b593336e4ec8d40a`. Cloudflare deployed version `acf06763-65a7-4814-b146-4a404a9ee21c`. Source is published through the existing pull-request branch; this record does not claim a merge into `main` or a current-source live AI response.


## Portrait camera repair — September 13, 2026

HandFrame and Invisible now share Cube’s native camera proportions throughout the compositor and recorder. Portrait use no longer requires fullscreen or a sideways phone. The normal page scrolls to Record and the design controls. See [behavior and recovery](PORTRAIT-CAMERA.md).

Local verification passed 283 application tests, 14 harness tests, frontend/Worker typechecks and build, asset verification, dependency audit and deployment dry-run. Browser checks passed: phone studio 31, photo tracking 14, Cube 21, straight panel 9 and native camera 9. The new camera suite independently measures round/square source objects in raster and CSS, front/rear mirror markers and all four camera edges; downloads and decodes actual 432×768 portrait and 768×432 landscape clips; verifies recording finalization before orientation resize; and checks Invisible room/countdown invalidation, photo contain, ordinary-page Record reachability and cleanup. The legacy tracking mock was corrected to describe its actual portrait camera and keep its physical hand proportions; the damping acceptance ratios remain unchanged.

An independent reviewer inspected the source, screenshots and decoded recordings and found no blocking issue. Browser camera/tracking inputs are synthetic. These checks do not establish physical iPhone gesture accuracy, native sharing, a Photos save or device performance. Those observations remain pending. No personal camera media is included in the repository, and no new upload or infrastructure binding was added.
